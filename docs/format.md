...
- Preserve the source token sequence except for formatter-owned structural changes: include sorting, trailing-comma normalization, inserting required control-statement braces, and safely concatenating adjacent ordinary string literals only when the resulting emitted line stays within the configured line width. A string literal that ends with escaped `\n`, including `\r\n`, is a line fragment boundary: it is not concatenated with the following literal, and adjacent line-fragment literals stay laid out as separate physical lines. Outside those formatter-owned changes, the formatter only changes spaces and line breaks.
