...
- Wrapped stream-shift chains using `<<` or `>>` keep the stream or left-hand receiver on the first line, then put each continued `<< value` or `>> value` segment at the beginning of a continuation line indented one level. This applies even when the chain is a standalone statement.
