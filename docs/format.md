...
- Format pointer and reference declarators left-bound to the type: `Type* value`, `const Type& value`, `const Type&`, `Type&& value`, `mutable Target* cached`, `virtual State& Method() = 0`, `const auto& [name, value]`, `void* (*callback)(...)`, `bool (*predicate)(...)`, and `reinterpret_cast<ColorConfig*>(address)`.
