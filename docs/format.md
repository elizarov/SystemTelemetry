...
- Format pointer and reference declarators left-bound to the type, including typedef-style names, lowercase system types, globally-qualified types, managed C++ handles, tracking references, and calling-convention function pointers: `Type* value`, `const Type& value`, `const Type&`, `uint16_t& encodedIndex`, `const sockaddr* address`, `void*`, `Type&& value`, `mutable Target* cached`, `virtual State& Method() = 0`, `::Renderer& renderer`, `const auto& [name, value]`, `ManagedType^ handle`, `ManagedType^% trackingRef`, `ManagedType% reference`, `void* (*callback)(...)`, `bool (*predicate)(...)`, `ZeResult(__cdecl*)(std::uint32_t)`, `reinterpret_cast<ColorConfig*>(address)`, and `reinterpret_cast<IUnknown**>(address)`.
