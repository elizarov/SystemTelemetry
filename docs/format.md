...
- Wrapped stream-shift chains using `<<` or `>>` keep the stream or left-hand receiver on the first line, then put each continued `<< value` or `>> value` segment at the beginning of a continuation line indented one level. This applies even when the chain is a standalone statement. Stream configuration methods listed in `tools\format_config.json`, such as `std::left`, `std::setw`, `std::fixed`, and `std::setprecision`, stay on the same continuation line as the following streamed value when the combined line fits.
