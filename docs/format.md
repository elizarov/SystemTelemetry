...
- Separate multi-line type aliases from neighboring declarations with one empty line, even when the neighbors are fields. Consecutive fields stay in one field group when wrapping only moves the initializer to a continuation line. A field initializer that owns a multi-line delimiter list with its closing `}`, `)`, or `]` back at the field indentation separates from neighboring fields for readability.
