...
- Format pointer and reference declarators left-bound to the type, including managed C++ handles, tracking references, and calling-convention function pointers: `Type* value`, `const Type& value`, `const Type&`, `void*`, `Type&& value`, `mutable Target* cached`, `virtual State& Method() = 0`, `const auto& [name, value]`, `ManagedType^ handle`, `ManagedType^% trackingRef`, `ManagedType% reference`, `void* (*callback)(...)`, `bool (*predicate)(...)`, `ZeResult(__cdecl*)(std::uint32_t)`, `reinterpret_cast<ColorConfig*>(address)`, and `reinterpret_cast<IUnknown**>(address)`.
