...
- Format pointer and reference declarators left-bound to the type, including managed C++ handles and tracking references: `Type* value`, `const Type& value`, `const Type&`, `void*`, `Type&& value`, `mutable Target* cached`, `virtual State& Method() = 0`, `const auto& [name, value]`, `ManagedType^ handle`, `ManagedType^% trackingRef`, `ManagedType% reference`, `void* (*callback)(...)`, `bool (*predicate)(...)`, `reinterpret_cast<ColorConfig*>(address)`, and `reinterpret_cast<IUnknown**>(address)`.
