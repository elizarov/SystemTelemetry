...
- Verify interactive layout-edit tooltips through an `/edit-layout` UI run, hover supported metric-list, throughput, drive-usage, and gauge guides or non-font anchors, confirm the standard Windows tooltip can extend beyond the dashboard window, confirm the first line uses `[section] parameter = value`, and confirm font anchors plus container split guides do not show a tooltip.
