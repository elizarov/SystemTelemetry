...
- Verify one headless `/trace /default-config /edit-layout:throughput /screenshot /exit` run when validating the throughput widget-local guide, and confirm the screenshot includes the throughput outline plus its `axis_padding` guide at the plot-left edge without needing live hover.
- Verify widget-local edit guides through an interactive `/edit-layout` UI run, hover `metric_list`, `throughput`, and `drive_usage_list`, and confirm the widget outline plus the `label_width`, `axis_padding`, `activity_width`, and `free_width` guides appear inside those widgets and drag live.
