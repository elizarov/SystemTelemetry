...
- NVIDIA FPS tracing reports `fps_etw:*` startup, provider-enable, sampled-present, and shutdown markers when the ETW session can be opened. NVIDIA FPS sample diagnostics include cumulative source-event counts and the selected process window count. If Windows denies ETW access, diagnostics mark `gpu.fps` with the `permission_required` issue and record the Win32 failure in the NVIDIA provider diagnostics.
