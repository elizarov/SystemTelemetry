...
- Verify interactive layout-edit tooltips through an `/edit-layout` UI run, hover supported metric-list, throughput, drive-usage, and gauge guides plus both non-font and font anchor handles, and also hover widget text runs plus card title text, confirm the standard Windows tooltip can extend beyond the dashboard window, confirm the first line uses `[section] parameter = value` for normal parameters and `[fonts] key = face,size,weight` for font anchor handles, confirm the tooltip is shown only if pressing mouse down changes parameter, and confirm hovered text-target highlight regions and container split guides do not show a tooltip.
