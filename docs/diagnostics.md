...
- Verify card-style edit chrome through an `/edit-layout` UI run, hover several cards including one with a header icon and one headerless card, and confirm each hovered card shows the shared `card_padding` top guide, `card_radius` square corner anchor, and `card_border` top circle anchor, confirm headered cards also show the `header_content_gap` guide and cards with both icon and title also show the `header_icon_gap` guide, confirm hovering a header icon adds the dotted icon highlight plus the square `header_icon_size` anchor, and confirm those card-style guides and anchors remain visible while a widget guide or card title text inside the same card is hovered.
...
- Verify interactive text editing through an `/edit-layout` UI run, hover several rendered text runs inside different widgets plus card title text, confirm the dotted per-text highlight and upper-right anchor appear on hover, and confirm dragging that anchor left or right updates the corresponding shared `[fonts]` size live.
...
- Verify interactive layout-edit tooltips through an `/edit-layout` UI run, hover supported card-style, metric-list, throughput, drive-usage, and gauge guides, container split guides, both non-font and font anchor handles, and also hover widget text runs plus card title text, confirm the standard Windows tooltip can extend beyond the dashboard window, confirm the first line uses `[section] parameter = value` for normal parameters, `[fonts] key = face,size,weight` for font anchor handles, and `[card.<id>] layout = ... rows(...)` or `columns(...)` or `[layout.<name>] cards = ... rows(...)` or `columns(...)` for container split guides with the two neighboring children and their current weights, confirm the container split guide second line uses the shared localized help text, confirm the tooltip is shown only if pressing mouse down changes parameter, and confirm hovered text-target highlight regions do not show a tooltip.
