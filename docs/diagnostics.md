...
- Verify widget-local edit guides through an interactive `/edit-layout` UI run, hover `metric_list`, `throughput`, and `drive_usage_list`, and confirm the widget outline plus the `label_width`, `axis_padding`, `header_gap`, `activity_width`, and `free_width` guides appear inside those widgets and drag live.
