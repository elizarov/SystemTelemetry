...
- App-icon exports use the same programmatic icon renderer as the live window, tray, and dialog icons, write PNG files directly from the generated icon bitmap, use the current resolved theme colors including the dashboard card fill composited over the app background, and do not depend on dashboard render scale.
...
- The current dump format version is `casedash_snapshot_v11`.
