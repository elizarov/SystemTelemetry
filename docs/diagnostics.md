...
- Trace output appends UTF-8 text without a BOM and uses the `[trace yyyy-mm-dd hh:mm:ss.mmm] <prefix>:` format. Supported prefix-filter names are `amd_adlx`, `asus_armoury_crate`, `board_vendor`, `crash`, `display_placement`, `diagnostics`, `fake`, `fps_etw`, `fps_provider`, `fps_service_client`, `gigabyte_siv`, `gpu_vendor`, `intel_level_zero`, `lenovo_diagnostics_driver`, `layout_edit_drag`, `msi_center`, `nvidia_nvml`, `profile`, `renderer`, `telemetry`, `unsupported_board`, `unsupported_gpu`, and `wallpaper`.
...
- The diagnostics trace covers startup, reload, output export, renderer layout data, runtime timing summaries, telemetry collection, hardware-provider activity, powered-off GPU FPS zero decisions, presented-FPS service client probes, presented-FPS ETW probes, unsupported-provider markers, and summarized layout-edit drag profiling when `/trace` is enabled. Provider-specific trace markers are listed in [docs/hardware.md](hardware.md).
