...
- Verify one headless `/trace /default-config /edit-layout:gauge /screenshot /exit` run when validating the gauge widget-local guide, and confirm the screenshot includes the gauge outline plus its right-end diagonal `sweep_degrees` guide without needing live hover.
