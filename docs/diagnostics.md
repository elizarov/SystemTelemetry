...
- Verify one headless `/trace /default-config /edit-layout:drive_usage_list /screenshot /exit` run when validating the drive-usage widget-local guides, and confirm the screenshot includes the drive-usage outline plus its `bar_gap`, `value_gap`, `activity_width`, `free_width`, `header_gap`, and `row_gap` guides together with the centered diamond `activity_segments` anchor without needing live hover.
- Verify widget-local edit guides through an interactive `/edit-layout` UI run, hover `gauge`, `metric_list`, `throughput`, and `drive_usage_list`, and confirm the widget outline plus the gauge `sweep_degrees` and `segment_gap_degrees` radial guides, top-edge diamond `segment_count` anchor, `label_width`, metric-list `vertical_gap`, `axis_padding`, `header_gap`, `bar_gap`, `value_gap`, `activity_width`, `free_width`, drive-usage `header_gap`, `row_gap`, and centered `activity_segments` anchor appear inside those widgets and drag live.
