...
- App-icon exports use the same programmatic icon renderer as the live window, tray, and dialog icons, use the current resolved theme colors including the dashboard card fill composited over the app background, and do not depend on dashboard render scale.
