...
- Verify widget-local edit guides through an interactive `/edit-layout` UI run, hover `metric_list`, `throughput`, and `drive_usage_list`, and confirm the widget outline plus the `label_width`, metric-list `vertical_gap`, `axis_padding`, `header_gap`, `activity_width`, `free_width`, drive-usage `header_gap`, and drive-usage `row_gap` guides appear inside those widgets and drag live.
- Verify interactive text editing through an `/edit-layout` UI run, hover several rendered text runs inside different widgets, confirm the dotted per-text highlight and upper-right anchor appear on hover, and confirm dragging that anchor left or right updates the corresponding shared `[fonts]` size live.
