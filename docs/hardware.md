...
- GPU telemetry selects the supported hardware provider from the configured GPU adapter selection identity. Empty GPU configuration selects the first unique non-software DXGI adapter.
...
- GPU adapter selection names preserve the raw DXGI display name when it is unique. When multiple unique adapters share the same display name, their selection names append ` #1`, ` #2`, and later numbers assigned by usable PCI address before DXGI adapter index. The numbered selection name is the value used by the dashboard devices menu and `[gpu] adapter_name`, while the raw display name stays in `GpuAdapterInfo` for vendor-provider matching.
...
- The unelevated dashboard queries `CashDashService` `board_sensors_sample` so the LocalSystem service can run the same direct diagnostics-driver path. When the service is absent or unreachable in a non-elevated dashboard, requested Lenovo temperature values and any requested fan values still missing after the direct GameZone WMI probe are marked `!admin`.
- Elevated runs without the service query the diagnostics-driver path directly in the dashboard process.
- Trace output can include `lenovo_diagnostics_driver:*` provider details, `driver_cpu_temperature_*` markers for direct CPU temperature reads, `direct_snapshot_refresh_started` markers for synchronous direct reads, `gamezone_wmi_*` markers for fan queries, and `unsupported_board` markers.
