...
- `src/layout_edit/` is the runtime layout-edit interaction layer. It may depend on layout_edit, dashboard_renderer, layout_model, widget, config, and util modules, but must not depend on dashboard, diagnostics, display, layout-edit dialog, main, telemetry, Direct2D, DirectWrite, WIC, or WRL modules.
- `src/layout_edit_dialog/` is the modeless layout-editor dialog layer. It may depend on layout_edit_dialog, config, dashboard_renderer, layout_edit, layout_model, telemetry, util, and widget modules, but must not depend on dashboard, diagnostics, display, main, renderer, Direct2D, DirectWrite, WIC, or WRL modules.
...
- `src/layout_edit/` is the runtime layout-edit interaction layer, so layout-edit modules may depend on layout_edit, config, dashboard_renderer, layout_model, util, and widget modules but must not depend on other project modules or the synthetic `d2d` package.
- `src/layout_edit_dialog/` is the modeless layout-editor dialog layer, so layout-edit-dialog modules may depend on layout_edit_dialog, config, dashboard_renderer, layout_edit, layout_model, telemetry, util, and widget modules but must not depend on other project modules or the synthetic `d2d` package.
