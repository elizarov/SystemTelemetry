...
- `src/tools/` contains native repository tools built into `build\CaseDashTools.exe`, including the maintained combined lint scanner behind `lint.cmd` and the tree-sitter formatter subcommand behind `format.cmd`. `src/tools/vendor/tree-sitter/` contains the vendored C and C++ grammar sources and generated parser files compiled into the formatter. `tools/` contains script entrypoints for formatting, profiling, generated assets, grammar regeneration, and compatibility helpers. `tools/lint_config.json` owns lint policy data, and `tools/format_config.json` owns native formatter constants, include-sorting groups, and project macro categories consumed by the grammar regeneration tool.
