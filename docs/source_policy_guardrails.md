...
- Source literals stay narrow UTF-8 by default. Fixed UTF-16 literals are allowed only as documented `const wchar_t` boundary constants for wide-native boundaries with no A-style API, such as DirectWrite, WIC filename methods, Desktop Wallpaper COM, CLR string callbacks, and fixed Windows structures whose fields are UTF-16.
