...
`[display]` selects the target monitor, active named layout, wallpaper, placement, autohide edge, and optional explicit render scale. Runtime display scale resolution rounds to three decimal places before sizing the dashboard. A non-empty `wallpaper` value represents committed CaseDash wallpaper ownership only when the placement resolves to the target monitor origin and the resolved scale makes the dashboard fill that monitor. Manual move and resize can leave a stale in-memory wallpaper value until a commit path runs; `Save Config` and full config export normalize stale display wallpaper ownership to `wallpaper = `. `autohide` is empty by default and is written by `Configure Display`: fullscreen clears it, while edge placements write `top`, `left`, `bottom`, or `right`.
