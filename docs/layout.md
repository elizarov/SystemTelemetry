...
When the pointer is outside every card in layout-edit mode, every dashboard `rows(...)` and `columns(...)` container also shows one gap anchor for its first rendered gap, using `[dashboard].row_gap` or `[dashboard].column_gap`.
...
When the pointer is anywhere inside a rendered card in layout-edit mode, every `rows(...)` and `columns(...)` container resolved inside that card also shows one gap anchor for its first rendered gap, using `[card_style].row_gap` or `[card_style].column_gap`.
