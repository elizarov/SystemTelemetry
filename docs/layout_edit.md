...
- Empty board-metric bindings are auto-detected from the provider sensor-name list: `cpu` and `gpu` logical bindings use the first sensor name containing the same term, and `system` logical bindings use the first fan sensor name containing `system` or `sys`. Auto-detected bindings become live configuration and are saved by the next `Save Config`.
