...
- `format.cmd` is the maintained entrypoint for formatting non-vendored C++ sources. It builds `build\CaseDashTools.exe` through `build.cmd /tools` when the tool is missing, then delegates to `CaseDashTools.exe format`. Its changed-file mode keeps Git CRLF normalization warnings out of formatter file discovery. `format.cmd --path <file-or-directory>` checks a limited eligible file or directory scope, and `format.cmd fix --path <file-or-directory>` rewrites only that limited scope so formatter behavior can be tested incrementally. `format.cmd --root <path> --file <path>` checks one eligible file relative to an alternate root, and `--stdout` with `--file` emits the formatter result without writing the file so formatter fixtures can compare golden output. Successful check and fix summaries report elapsed time.
- `CaseDashTools.exe format` is the tree-sitter formatter subcommand behind `format.cmd`. It scans non-vendored `.cpp` and `.h` files under `src\` and `tests\`, including mixed-mode C++/CLI provider bridges, supports `fix`, `changed`, `--root <path>`, `--file <path>`, `--path <file-or-directory>`, `--stdout`, and `--verbose`, parses each file through the vendored tree-sitter C++ grammar compiled into `CaseDashTools`, applies the formatter rules encoded in `src\tools\format.cpp`, and reports elapsed time plus the number of files whose tree-sitter parse contains errors. Project-specific format constants, macro names consumed when regenerating grammar outputs, and include-sorting group definitions live in `tools\format_config.json`; formatting style rules remain hardcoded in the formatter source.
- `tools\regenerate_tree_sitter_grammar.py` is the explicit maintenance tool for regenerating vendored tree-sitter C++ grammar outputs after editing `src\tools\vendor\tree-sitter\tree-sitter-cpp\grammar.js` or parser macro categories in `tools\format_config.json`. It writes `case_dash_macro_config.js`, runs the pinned tree-sitter CLI, and keeps generated parser files under `src\tools\vendor\tree-sitter\tree-sitter-cpp\src\`. If no CLI path is provided, it downloads the pinned Windows CLI under `build\`.
