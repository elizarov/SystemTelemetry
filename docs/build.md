...
- `CaseDashTools.exe format` is an experimental native formatter subcommand for timing and evaluating a tree-sitter C++ alternative to the maintained `format.cmd` path. It uses the same broad eligible-file filter, supports `fix`, `changed`, `--root <path>`, `--file <path>`, and `--stdout`, parses each file through the packaged tree-sitter C++ grammar, applies the formatter rules encoded in `src\tools\format.cpp`, and reports elapsed time plus the number of files whose tree-sitter parse contains errors. Its output is intentionally simple and not clang-format-compatible.
