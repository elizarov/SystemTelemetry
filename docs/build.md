...
- The active LLVM source-analysis tool is `clangd` `20.1.8`.
...
- `format.cmd` is the maintained entrypoint for repository formatting. It builds `build\CaseDashTools.exe` through `build.cmd /tools` when the tool is missing, collects maintained `.cpp` and `.h` files under `src\` and `tests\`, and delegates the discovered newline-separated file list to `CaseDashTools.exe format --files`. `format.cmd changed` limits discovery to changed and untracked files, `format.cmd fix` rewrites all discovered files, and `format.cmd fix staged --restage` is the pre-commit staged formatting path. Pass `--concurrency <n>` to override native formatter worker count. Successful check and fix summaries report completed file counts plus elapsed time.
- `CaseDashTools.exe format` is the tree-sitter formatter subcommand behind `format.cmd` and clang-format-like IDE integration. It reads stdin when no file input is present, accepts direct file arguments and newline file lists through `--files <path>` or `--files=<path>`, supports `-i`, `--dry-run` or `-n`, `--style=file`, `--style=<path>`, `--style=file:<path>`, `--concurrency <n>`, and `--verbose`, locates `.cpp-format` and `.cpp-format-ignore` by walking upward from each file, caches loaded config and ignore files, parses each source through the vendored tree-sitter C++ grammar compiled into `CaseDashTools`, rejects sources whose parse contains errors or missing nodes, applies the fixed formatter rules encoded in `src\tools\format.cpp` and internal `src\tools\impl\format_*` formatter modules, and reports completed file counts plus elapsed time. `.cpp-format` owns only the formatter inputs that are intentionally configurable; formatting style rules remain hardcoded in the formatter source.
- `CaseDashTools.exe format_model_dump <source-file>` is the formatter model inspection command. It parses one C++ source file with the same tree-sitter model path as the formatter and writes a two-space-indented YAML `SyntaxNode` tree to stdout for debugging.
- `tools\regenerate_tree_sitter_grammar.py` is the explicit maintenance tool for regenerating vendored tree-sitter C++ grammar outputs after editing `src\tools\vendor\tree-sitter\tree-sitter-cpp\grammar.js` or parser macro categories in `.cpp-format` or `tools\tests\format\.cpp-format-userver`. It writes `macro_config.js`, runs the pinned tree-sitter CLI, and keeps generated parser files under `src\tools\vendor\tree-sitter\tree-sitter-cpp\src\`. If no CLI path is provided, it downloads the pinned Windows CLI under `build\`.
- `test.cmd` is the maintained entrypoint for running the CTest suite against the built Release tree, the formatter regression fixture under `tools\tests\format`, and the lint-check regression fixture under `tools\tests\lint`. It prints verbose CTest output so GitHub runner logs show each test command and its stdout even when tests pass.
- The repo `pre-commit` hook launches `tools\pre_commit_checks.ps1` and runs `lint.cmd` before each commit. The staged formatter step is temporarily skipped while formatter break selection is still under implementation; when re-enabled, it runs `format.cmd fix staged --restage` before lint. The hook temporarily stashes unstaged and untracked files while it checks the staged snapshot, restores them before exit, and aborts the commit when checks fail.
- `lint.cmd` is the maintained entrypoint for architecture checks, source dependency checks, include-path checks, no-local-`NOLINT` checks, header-body checks, renderer-only Direct2D boundary checks, and source-policy checks such as the project-wide `std::function`, `std::filesystem`, STL threading primitive, `std::hash`, and wide-literal bans. It runs `build\CaseDashTools.exe lint_check --config <repo>\tools\lint_config.json --check` from the repository root, building `CaseDashTools` first only when the executable is missing, non-vendored tool source files are newer than the executable, or `CMakeLists.txt` is newer than the generated `build\cmake\build.ninja` graph. The native `lint_check` command supports `--config <path>`, `--check`, `--concurrency <n>`, `--no-progress`, `--report-json <path>`, and `-v` or `--verbose`; `lint.cmd` forwards `--concurrency <n>` to that command, and graph-output arguments are not supported. `tools\lint_config.json` owns shared suffix groups, lint scan roots, checker eligibility, exclusions, include roots, allowlists, package dependency limits, and checker-owned source-policy or suppression rules. Clean runs print one timed success line with the completed file count and total LOC; failing runs report findings plus the timed failure line. Pass `--report-json <path>` to write the same failures as a machine-readable JSON report with `schema_version`, `failed`, and `diagnostics` fields. Pass `-v` or `--verbose` to `lint.cmd` or `CaseDashTools.exe lint_check` for source files above the configured LOC threshold and DAG topological package dependency details. The source-policy rationale lives in [docs/source_policy_guardrails.md](source_policy_guardrails.md).
...
- `devenv.cmd` is the maintained environment bootstrap for local builds and tool runs. GitHub Actions does not use this machine-local script; `build.cmd` and `format.cmd` resolve Visual Studio and package-managed native tool dependencies from the runner environment, while `lint.cmd includes` resolves LLVM clangd from the runner environment.
