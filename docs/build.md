...
- `lint.cmd` is the maintained entrypoint for architecture checks, source dependency graph checks, include-path checks, header-body checks, and optional `clang-tidy` runs. Each lint run rebuilds the source dependency DOT and GraphML under `build\architecture\` without rendering SVG. The optional tidy sweep checks maintained non-vendored `.cpp` and `.h` files except the Gigabyte SIV provider bridge files, writes `build\clang_tidy_report.txt`, uses a three-minute per-file timeout, reports unused internal functions as errors, reports unused includes as errors, and filters maintained include-cleaner false positives.
