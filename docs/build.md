...
- `lint.cmd` is the maintained entrypoint for architecture checks, source dependency graph checks, include-path checks, no-local-`NOLINT` checks, header-body checks, renderer-only Direct2D boundary checks, and source-policy checks such as the project-wide `std::function`, `std::filesystem`, STL threading primitive, `std::hash`, and wide-literal bans. It runs `tools\lint_check.py`, which discovers lint inputs once, updates one scan-progress line in an interactive console, prints per-file progress in redirected logs, and reuses the same file contents for the architecture, source dependency, include-style, and source-policy reports. The source-policy rationale lives in [docs/source_policy_guardrails.md](source_policy_guardrails.md). Each lint run rebuilds the source dependency DOT and GraphML under `build\architecture\` without rendering SVG.
