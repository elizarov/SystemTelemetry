...
- `lint.cmd` is the maintained entrypoint for architecture checks, source dependency graph checks, include-path checks, no-local-`NOLINT` checks, header-body checks, renderer-only Direct2D boundary checks, and source-policy checks such as the project-wide `std::function`, `std::filesystem`, STL threading primitive, `std::hash`, and wide-literal bans. It runs `tools\lint_check.py`, which loads `tools\lint_config.json`, discovers lint inputs once, updates one scan-progress line in an interactive console, reads and parses each file once, and dispatches the shared per-file record to the map-reduce checkers under `tools\lint\`. `tools\lint_config.json` owns shared suffix groups, lint scan roots, checker eligibility, exclusions, include roots, allowlists, package dependency limits, graph output defaults, and checker-owned source-policy or suppression rules. The checkers keep file-specific work in `process_file` and reserve only cross-file reductions and graph output writes for `finish`, so scan progress reflects the real work breakdown. Clean runs print one total-LOC summary; failing runs report only findings or graph-write errors. Pass `-v` or `--verbose` to `lint.cmd` or `tools\lint_check.py` for source files above the configured LOC threshold and DAG topological package dependency details. The source-policy rationale lives in [docs/source_policy_guardrails.md](source_policy_guardrails.md). Each lint run rebuilds the source dependency DOT and GraphML under `build\architecture\` without rendering SVG.
