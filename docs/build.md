...
- `CaseDashTools.exe format` is the tree-sitter formatter subcommand behind `format.cmd` and clang-format-like IDE integration. It reads stdin when no file input is present, accepts direct file arguments and newline file lists through `--files <path>` or `--files=<path>`, supports `-i`, `--dry-run` or `-n`, `--style=file`, `--style=<path>`, `--style=file:<path>`, and `--verbose`, locates `.cpp-format` and `.cpp-format-ignore` by walking upward from each file, caches loaded config and ignore files, parses each source through the vendored tree-sitter C++ grammar compiled into `CaseDashTools`, applies the fixed formatter rules encoded in `src\tools\format.cpp` and internal `src\tools\impl\format_*` formatter modules, and reports elapsed time plus the number of files whose tree-sitter parse contains errors. `.cpp-format` owns only the formatter inputs that are intentionally configurable; formatting style rules remain hardcoded in the formatter source.
