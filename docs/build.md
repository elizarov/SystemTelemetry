...
- `tools\regenerate_tree_sitter_grammar.py` is the explicit maintenance tool for regenerating vendored tree-sitter C++ grammar outputs after editing `src\tools\vendor\tree-sitter\tree-sitter-cpp\grammar.js` or parser macro categories in `.cpp-format`. It writes `case_dash_macro_config.js`, runs the pinned tree-sitter CLI, and keeps generated parser files under `src\tools\vendor\tree-sitter\tree-sitter-cpp\src\`. If no CLI path is provided, it downloads the pinned Windows CLI under `build\`.
