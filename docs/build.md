...
- `test.cmd` is the maintained entrypoint for running the CTest suite against the built Release tree and the native lint-check regression fixture under `tools\tests\lint`. It prints verbose CTest output so GitHub runner logs show each test command and its stdout even when tests pass.
