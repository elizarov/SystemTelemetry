...
- `lint.cmd` is the maintained entrypoint for architecture checks, source dependency graph checks, include-path checks, header-body checks, and optional `clang-tidy` runs. The optional tidy sweep writes `build\clang_tidy_report.txt`, uses a one-minute per-file timeout, filters maintained include-cleaner false positives, and excludes `src\diagnostics\snapshot_dump.cpp` because the active Clang diagnostics stall on that translation unit.
