...
- Development builds set the Windows version resource prerelease flag and are identified by the generated version constants used by the About dialog and release assets.
...
`release.cmd` asks for keyboard confirmation, stamps or validates the top changelog header, updates [VERSION](../VERSION) when needed, commits `VERSION` and `docs/changelog.md` when either file changes, runs format, lint, `build.cmd /benchmarks`, and tests, creates the matching annotated `v<VERSION>` tag, pushes the current branch, and pushes the tag.
