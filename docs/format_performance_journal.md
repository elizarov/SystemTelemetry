...
- Remaining hotspots: tree-sitter parse, `Printer::FlushPendingTokens`, `BuildFormatModel`, and token spacing. Serial `format-all` phase totals now line up with wall-clock `format_loop`, while interactive tool runs stay parallel by default.
