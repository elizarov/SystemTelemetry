...
- Measure all-source formatter check throughput with `build\CaseDashBenchmarks.exe format-all 3 2`; the benchmark formats every maintained `.cpp` and `.h` file under `src` and `tests` in memory with the same ignore and style discovery as `format.cmd`, reports whether every file is already formatted, and excludes file writes. Per-phase formatter splits are cumulative across worker threads, so they show CPU work and can exceed wall-clock `format_loop`.
- Measure formatter golden throughput with `build\CaseDashBenchmarks.exe format-golden 20 2`; the benchmark formats `tools\tests\format\src\format_test_input.cpp` in memory and reports total plus per-iteration time without file writes. It also reports formatter phase splits for parse, print, tokenization, break-model construction, DP solve, output emission, and source comparison.
...
- Capture a benchmark-focused CPU profile with `profile_benchmark.cmd animation 2400 2`, `profile_benchmark.cmd edit-layout 240 2`, `profile_benchmark.cmd format-all 3 2`, `profile_benchmark.cmd format-golden 20 2`, `profile_benchmark.cmd layout-switch 240 2`, `profile_benchmark.cmd mouse-hover 240 2`, `profile_benchmark.cmd snapshot-handoff 240 2`, `profile_benchmark.cmd telemetry-init 2 2`, `profile_benchmark.cmd theme-change 240 2`, or `profile_benchmark.cmd update-telemetry 240 2` when a change materially moves that benchmark or when hotspot confirmation is needed.
