...
- Measure Lenovo LDE thermal API layers with `build\CaseDashBenchmarks.exe lenovo-lde-phases 1 2` or, for privileged Lenovo diagnostics access, `profile_benchmark.cmd /direct lenovo-lde-phases 1 2` through the elevated daemon. The benchmark creates separate cold LdeApi runtimes for manual CPU thermal-tool execution, `GetAvailableModules` followed by the same manual execution, and normal CPU `LoadModules` followed by execution. Use it when deciding whether a lower LDE access path avoids cold server or module setup.
- Measure Lenovo Hardware Scan thermal execution with `build\CaseDashBenchmarks.exe lenovo-hardware-scan 3 2` or, for privileged Lenovo diagnostics access, `profile_benchmark.cmd /direct lenovo-hardware-scan 3 2` through the elevated daemon. The benchmark loads only the CPU temperature module, keeps one Lenovo runtime alive across iterations, and prints cold setup time, warm sample time, LdeApi phase timings, raw execution events, and returned package or per-core temperatures.
...
- Capture a benchmark-focused CPU profile with `profile_benchmark.cmd animation 2400 2`, `profile_benchmark.cmd edit-layout 240 2`, `profile_benchmark.cmd layout-switch 240 2`, `profile_benchmark.cmd lenovo-gamezone 5 2`, `profile_benchmark.cmd lenovo-lde-phases 1 2`, `profile_benchmark.cmd lenovo-hardware-scan 3 2`, `profile_benchmark.cmd mouse-hover 240 2`, `profile_benchmark.cmd snapshot-handoff 240 2`, `profile_benchmark.cmd telemetry-init 2 2`, `profile_benchmark.cmd temperature-sources 1 2`, `profile_benchmark.cmd theme-change 240 2`, or `profile_benchmark.cmd update-telemetry 240 2` when a change materially moves that benchmark or when hotspot confirmation is needed.
...
- `lenovo-lde-phases` measures candidate private or lower LDE paths as separate cold starts. Use it to prove whether manual thermal-tool payloads or available-module queries avoid public `LoadModules` cost before carrying a private integration forward.
- `lenovo-hardware-scan` separates cold Lenovo LdeApi/module setup from repeated thermal execution. Use it when temperature startup is slow: on affected systems a cold CPU-only scan can take seconds while the same warmed runtime can return package and per-core temperatures in tens of milliseconds. The benchmark phase timings distinguish managed bridge setup, first `LoadModules` invocation, task waits, execution invocation, and total capture time so redundant LdeApi requests do not get mistaken for the thermal read cost.
