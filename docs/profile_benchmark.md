...
- Measure all-source formatter CPU work with `build\CaseDashBenchmarks.exe format-all 3 2`; the benchmark formats every maintained `.cpp` and `.h` file under `src` and `tests` in memory with the same ignore and style discovery as `format.cmd`, reports whether every file is already formatted, excludes file writes, and forces concurrency `1` so phase totals describe serial formatter work instead of parallel wall-clock throughput.
