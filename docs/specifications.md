...
- Text-widget preferred heights must come from the dedicated `[fonts].text` metrics plus `[text].bottom_padding`, `network_footer` preferred heights must come from the dedicated `[fonts].footer` metrics plus `[network_footer].preferred_padding`, and `vertical_spacer(widget_name)` preferred heights must match the preferred height of the referenced widget type.
...
- The `text` widget must expose one widget-local horizontal edit guide for `[text].bottom_padding` while the widget is hovered in layout-edit mode, drawn at the bottom edge of the widget so dragging that guide down increases and dragging it up decreases the fixed extra space reserved below the text line.
