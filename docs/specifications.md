...
- Metric-text wedge anchors must use tooltip first lines formatted as `[metrics] metric_id = style,scale,unit,label` with the same config-file value shape used by `[metrics]`, and their second line must use one shared localized metric-definition description.
...
- Metric leaves in the unified layout-edit editor must show display style read-only, label always editable, unit editable except for `label_only`, and scale editable only when the metric does not use telemetry-provided `*` scale and the style is not `label_only`.
