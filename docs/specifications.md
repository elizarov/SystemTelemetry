...
- The `gauge` widget must expose widget-local edit guides for `[gauge].sweep_degrees` and `[gauge].segment_gap_degrees` while the widget is hovered in layout-edit mode, with short diagonal radial guides drawn across the ring at the current right-end arc boundary and at the end of the first segment so dragging the sweep guide edits only the gauge end side and dragging the first-segment guide edits the apparent first-segment width while converting that movement back into the stored segment gap.
