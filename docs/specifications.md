...
- Widths that are fully determined by fixed renderer text such as throughput labels, throughput axis labels, drive-letter labels, and the `100%` drive percent column should be measured from the configured fonts at layout load, with only the throughput axis width widened by its dedicated widget-section padding entry.
...
- Throughput preferred height must come from the packed small-font header row, the configured `[throughput].header_gap`, and the plot max-label band height computed from `[throughput].scale_label_padding` and `[throughput].scale_label_min_height`, rather than from a dedicated graph-height config entry.
