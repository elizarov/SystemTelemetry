...
- While that popup menu or any direct-entry layout-edit dialog is open, layout-edit hover hit-testing, tooltip refresh, and cursor-shape overrides must stay suspended so the menu or dialog keeps the normal topmost cursor behavior instead of reflecting the dashboard content underneath.
