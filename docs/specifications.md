...
- While layout-edit mode is active, right-clicking while the pointer is over the same actionable widget-local guide, actionable gap-anchor handle, actionable edit-anchor handle, or container split guide must prepend one `Edit <item> ...` action to the popup menu for that exact hovered target, using a short description of the edited config item or layout weights, and choosing the direct-entry dialog by target type: numeric guides, gap anchors, and size anchors use a single-value number dialog, container split guides use a two-field neighboring-weight dialog, and font anchors use a font dialog that edits the stored face, size, and weight together with a dropdown list of installed font families; all of those dialogs must preview valid edits live into the dashboard while they stay open, and `Cancel` must restore the pre-dialog value or weights.
...
- Layout-edit mode must expose one maintained gap-anchor control for the first rendered gap of every `rows(...)` and `columns(...)` container in both the shared dashboard cards tree and every card layout tree, using `[dashboard].row_gap` or `[dashboard].column_gap` for dashboard containers and `[card_style].row_gap` or `[card_style].column_gap` for card containers.
...
- Dashboard gap anchors must stay visible only while the pointer is outside every card, and card gap anchors must stay visible only while the pointer is inside the corresponding rendered card.
