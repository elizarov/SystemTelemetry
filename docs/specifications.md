...
- Move mode keeps the dashboard attached to the pointer until placement completes, clamps the active pointer offset inside the current dashboard bounds after DPI or size changes, and overlays the current monitor name, current app scale percentage, monitor default scale percentage, logical relative coordinates, and dashboard size inside the same frame as the dashboard. Completing move mode updates the live display monitor and logical position once, so later layout or theme changes keep the placed position.
