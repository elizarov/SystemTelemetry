...
- The editor supports filtering, per-field revert, config-local descriptions, and specialized editors for numeric values, fonts, RGBA colors, metrics, weight pairs, and metric-list row ordering. Edit-dialog dropdowns open above neighboring controls with a popup sized for the available options, capped at ten visible rows.
