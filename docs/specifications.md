...
- GPU content includes model name, load, dedicated VRAM usage, total dedicated VRAM capacity, and hardware-provider metrics such as temperature, clock, fan speed, and game FPS when available from the active GPU provider. GPU fan speed falls back to a board-provider GPU fan only when the GPU provider does not supply fan RPM. Intel GPU temperature falls back to the resolved board CPU temperature only when the selected Intel GPU provider does not supply a native temperature; Intel GPU clock remains a selected-adapter graphics clock from Intel or Windows graphics APIs and does not reuse CPU clock. GPU telemetry selects the provider from the configured non-software adapter identity, or the first non-software adapter when no GPU adapter is configured. If no supported GPU provider matches, the unsupported GPU provider keeps provider metrics unavailable and supplies only the selected-adapter presented-FPS metric when ETW access and present events are available.
- Presented FPS is the default `gpu.fps` source when available. Runtime configs that do not request `gpu.fps` do not start the Presented FPS service or local ETW collection path. Process selection filters formatted GPU Engine 3D usage to the selected DXGI adapter LUID using the same adapter-activity source as `gpu.load`, prefers a presenting process with dominant selected-adapter 3D usage over background presenters, keeps the current presenter through brief count ties or near-ties, and reports `0 FPS` when the selected adapter is idle or powered off. When a dominant 3D application is visible on the selected adapter but matching present events are not, `gpu.fps` stays unavailable for that adapter.
- Board telemetry selects the provider from the baseboard manufacturer, reads supported board temperatures and fan speeds through the active provider, uses the CaseDash service when a supported provider needs privileged hardware access, marks affected values with `!admin` when that privileged service path is unavailable to a non-elevated dashboard, allows slow direct vendor fallbacks to refresh in the background while retaining the last successful values, and uses an unsupported-board provider with unavailable values when no supported board provider matches.
...
- The board mapping section connects those logical names to provider-specific sensor names. Empty CPU, GPU, and system bindings use first-use auto-detection from the active provider's sensor names; system bindings also match motherboard or board sensor names. Otherwise, bound board metrics resolve when the mapped sensor exists. The `gpu` board fan binding is requested by `gpu.fan` as a fallback source when the selected GPU provider does not expose fan RPM. The `cpu` board temperature binding is requested by `gpu.temp` so selected Intel GPUs can fall back to CPU package temperature when no native Intel GPU temperature exists. Supported provider details are defined in [docs/hardware.md](hardware.md).
