...
- The metric-list reorder editor page must list only generally available metric ids whose display style is `scalar`, `percent`, or `memory`, excluding special drive-list-only metric ids such as `drive.activity.read`, `drive.activity.write`, `drive.usage`, and `drive.free`.
