...
- When gauge preferred height is needed before that shared fitted radius exists, it must be derived from the configured gauge text offsets, ring thickness, outer padding, and measured gauge font metrics instead of a dedicated fixed-size config entry.
...
- While layout-edit mode is active, the `gauge` widget must also expose circular size anchors on the ring's right side, with one anchor on the outer ring edge that live-updates `[gauge].outer_padding` and one anchor on the inner ring edge that live-updates `[gauge].ring_thickness`.
