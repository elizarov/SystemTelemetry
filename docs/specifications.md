...
- While layout-edit mode is active, hovering anywhere inside a card must also activate that card's shared `[card_style]` chrome guides and anchors, even when the same pointer position is already activating widget-local guides or editable text inside that card.
...
- The shared `[card_style]` section must expose `card_padding`, `card_radius`, `card_border`, `header_icon_size`, `header_icon_gap`, `header_content_gap`, `column_gap`, and `row_gap` as the maintained card chrome and in-card spacing keys.
