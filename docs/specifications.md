...
- Embedded `resources/config.ini`, embedded `resources/localization.ini`, and executable-side `config.ini` inputs must be valid UTF-8 text, and invalid UTF-8 bytes must be rejected without decoding through the active ANSI code page.
...
- The shipped `[metrics]` section must include one generally available placeholder metric `nothing` whose label is `Nothing`, whose metric-list value renders as `N/A`, and which the metric-list add-row flow uses as the default newly inserted row selection.
...
- The shipped `[colors]` defaults must render card borders as subtle separators.
...
- When `display.monitor_name` targets a monitor that is not yet available during login startup, display-topology churn, or a temporary unplug, the UI must keep watching for that configured monitor and apply the saved logical position there once the monitor is available.
...
- Card-local layout expressions may reference another card id as a reusable sub-layout, and the renderer must substitute that referenced card's layout during layout resolution.
...
- When a `metric_list` or `drive_usage_list` widget has less vertical space than its full configured content needs, the renderer must keep each header, row, bar, and configured gap at its full configured height and crop any overflow at the bottom.
...
- When gauge preferred height is needed before that shared fitted radius exists, it must be derived from the configured gauge text bottom offsets, ring thickness, outer padding, and measured gauge text metrics.
...
- Dashboard display labels and units must come from `[metrics]`, with metric display formatting driven by the metadata-owned metric style for that metric id.
...
- While layout-edit mode is active, hovering a `metric_list` widget must also show one row-reorder anchor at the right edge of each visible row, vertically centered in that row and drawn as two outlined equilateral triangles with a small gap between them pointing up and down; dragging that anchor up or down must reorder the corresponding metric row and live-update the owning `[card.<id>] layout = metric_list(...)` or `[layout.<name>] cards = metric_list(...)` entry.
- While layout-edit mode is active, a `metric_list` widget that still has room to show one more row slot below its configured metrics must also show one `+` add-row anchor at the right edge of that first empty row slot; double-clicking that anchor or choosing `Add metric list row...` from its right-click menu must append one new metric row to the owning `metric_list(...)` configuration and open the unified editor focused on that same `metric_list` leaf so the new row can be changed from its dropdown.
- The unified layout-edit tree must also expose one `metric_list` leaf at the matching widget position inside each visible `cards` or `layout` container subtree, and selecting that leaf must open a metric-list reorder editor page.
- The metric-list reorder editor page must list only generally available metric ids whose display style is `scalar`, `percent`, or `memory`, excluding special drive-list-only metric ids such as `drive.activity.read`, `drive.activity.write`, `drive.usage`, and `drive.free`, and its metric dropdowns must unfold downward tall enough to show up to 10 visible options at once when enough items are available.
- Tree-selection changes and metric-list row button edits in the unified layout-edit dialog must update the right-hand pane without a full-dialog blink, using buffered tree repainting and local no-erase editor refresh so repeated navigation stays visually steady.
...
- Widget-local actionable hit-testing must use one shared maintained priority order that is visible in the grouped layout-edit parameter enum, with small actionable handles first, shared dashboard and card gap-anchor handles including the dashboard `outer_margin` anchor before overlapping widget-local horizontal and vertical guides, shared `[card_style].row_gap` and `[card_style].column_gap` handles before overlapping shared card chrome guides such as `[card_style].header_content_gap`, large circular gauge ring handles after those, and container split guides always last.
...
- Diamond-shaped segment-count anchors must use a tight hit region that stays at the rendered diamond bounds.
...
- Card header icons must use the embedded white PNG resources, and the renderer must recolor those white icon pixels at runtime to the configured `[colors].icon_color`.
...
- Circular anchor handle hit-testing must follow the distance from the pointer to the rendered circle outline, using a small padded tolerance around that outline.
...
- While a container layout-edit guide drag is active, the renderer must use the same full-fidelity widget draw path used by ordinary repaint frames.
...
- Widget-guide edits whose valid range depends on other live config values must clamp in the layout-edit controller against the current config, so direct drags stay responsive while loaded configs and non-interactive edits keep only field-local safety clamps.
...
- Gauges must render the bound metric's configured label and unified formatted metric text while using the same segmented ring geometry and fill rules.
- Gauge and drive-usage segment layout resolution must keep every configured segment drawable even if a saved or indirectly derived gap would otherwise consume all remaining extent, reducing the visible segment to a hairline at worst.
...
- Throughput header labels must use their actual rendered text width when reserving the left side of the value row.
