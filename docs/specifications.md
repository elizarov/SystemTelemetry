...
- The `drive_usage_list` widget must expose widget-local edit guides for `[drive_usage_list].activity_width`, `[drive_usage_list].free_width`, `[drive_usage_list].header_gap`, and `[drive_usage_list].row_gap` while the widget is hovered in layout-edit mode, with the horizontal guide below the header editing the shared header gap and the horizontal guides below the rows editing the shared row gap live.
