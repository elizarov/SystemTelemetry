...
- The `gauge` widget must expose one widget-local edit guide for `[gauge].sweep_degrees` while the widget is hovered in layout-edit mode, with a diagonal radial guide drawn from the gauge center to the current right-end arc boundary so dragging that guide tracks the center-to-pointer angle, edits only the gauge end side, and converts that right-end movement back into the full configured sweep.
