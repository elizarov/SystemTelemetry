...
- Throughput preferred height must come from the packed small-font header row, the configured `[throughput].header_gap`, and the plot max-label band height computed from the measured small-font height plus `[throughput].scale_label_padding`, rather than from a dedicated graph-height config entry.
