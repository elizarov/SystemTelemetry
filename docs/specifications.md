...
- While a container layout-edit guide drag is active, the renderer must keep the live resized geometry, labels, and active drag overlays visible but may switch widget internals to a lighter drag-preview paint path that omits expensive decorative details such as panel icons, throughput history traces and markers, recent-peak ghosts, and antialiased filled gauge segments until the drag ends.
