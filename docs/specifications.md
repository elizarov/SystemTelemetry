...
- Text-widget preferred heights must come from the dedicated `[fonts].text` metrics plus `[text].preferred_padding`, while `network_footer` and `spacer` preferred heights must come from the dedicated `[fonts].footer` metrics plus `[network_footer].preferred_padding`.
...
- While layout-edit mode is active, hovering a rendered widget text run or card title text run must also draw a light dotted highlight tightly around that run's measured drawn bounds plus a small anchor at the run's upper-right corner, and dragging that anchor left or right must live-update the shared configured font-size entry used by that run.
...
- While layout-edit mode is active, hovering a `gauge` widget must also draw a small diamond-shaped anchor centered on the gauge's top edge, and dragging that anchor left or right must live-update `[gauge].segment_count`.
...
- The `drive_usage_list` widget must expose widget-local edit guides for `[drive_usage_list].activity_width`, `[drive_usage_list].free_width`, `[drive_usage_list].header_gap`, and `[drive_usage_list].row_gap` while the widget is hovered in layout-edit mode, with the horizontal guide below the header editing the shared header gap and the horizontal guides after each visible non-empty row editing the shared row gap live without extending into empty missing-row space, and it must also expose one horizontal diamond anchor centered in the read/write indicator band so dragging that anchor left or right live-updates `[drive_usage_list].activity_segments`.
- The `gauge` widget must expose widget-local edit guides for `[gauge].sweep_degrees` and `[gauge].segment_gap_degrees` while the widget is hovered in layout-edit mode, with short diagonal radial guides drawn across the ring at the current right-end arc boundary and at the end of the first segment so dragging the sweep guide edits only the gauge end side and dragging the first-segment guide edits the apparent first-segment width while converting that movement back into the stored segment gap, and it must also expose the top-edge diamond anchor that drags `[gauge].segment_count` horizontally.
