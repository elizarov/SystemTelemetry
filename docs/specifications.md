...
- Row and column layout containers support child reorder handles. Row containers show up/down handles just inside the middle of each child slot's right side and drag vertically; column containers show left/right handles just inside the middle of each child slot's top side and drag horizontally. During a child reorder drag, the dragged child and its active handle track the pointer with the active dashed outline, the container underneath keeps the dragged child's current slot empty, the child order updates as soon as the pointer reaches another child slot, and the child snaps into its slot on release.
