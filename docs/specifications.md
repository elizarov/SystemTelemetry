...
- Clock time and date preferred heights must come directly from the measured configured font heights so small cards keep both lines visible without extra widget padding inflating the reservation.
...
- While layout-edit mode is active, the renderer must draw thin config-colored separator guides over every resolved `rows(...)` and `columns(...)` split, highlight the hovered non-empty widget by outlining its resolved widget box, show any supported widget-local size guides inside that hovered widget, and update the cursor to the matching horizontal or vertical resize shape when the pointer hovers a draggable guide.
...
- The `metric_list` widget must expose a widget-local edit guide for `[metric_list].label_width` while the widget is hovered in layout-edit mode, and dragging that guide must update the shared label-width config live so the label and bar split relayout immediately.
- The `throughput` widget must expose a widget-local edit guide for `[throughput].axis_padding` while the widget is hovered in layout-edit mode, with the guide drawn at the plot-left boundary between the vertical scale gutter and the plotted area so dragging it relayouts that shared gutter live.
- The `drive_usage_list` widget must expose widget-local edit guides for `[drive_usage_list].activity_width` and `[drive_usage_list].free_width` while the widget is hovered in layout-edit mode, and dragging either guide must update that shared config value live so both activity columns or the free-space column relayout immediately.
