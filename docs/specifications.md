...
- Dragging the hover titlebar starts dashboard move mode with the usual monitor, scale, and coordinate overlay. The titlebar close button exits through the same save-or-discard prompt used by `Exit` when layout-edit changes are unsaved.
