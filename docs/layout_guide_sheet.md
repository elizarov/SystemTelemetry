...
6. Promote top and bottom callouts. Try each possible left-stack item as the bottom callout and each possible right-stack item as the top callout, then choose the deterministic pair that yields the fewest straight-leader intersections.
...
10. Validate side order. After placement, check each left and right stack for same-side leader intersections and swap adjacent bubbles until no same-side leader intersections remain or the deterministic retry limit is reached.
