...
7. Repair side split. If the best promoted and ordered layout still has a nonzero leader score, try deterministic left/right swaps and one-item transfers, rerun promotion and stack-order scoring for each candidate split, and keep the candidate that moves the score toward zero.
...
14. Emit diagnostics. Trace the selected card ids, covered widget types, documented kind count, bubble count, representative target ids, per-card leader score, side-repair pass count, canvas size, and every placement warning.
