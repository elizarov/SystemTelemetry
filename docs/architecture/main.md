...
- Choose either the normal UI path or the headless diagnostics path.
...
- Keep the executable manifest disabling file virtualization so config reads and writes target the executable-side location.
