...
- Treat HWND size changes as synchronized presentation boundaries: the shell waits for the render thread to discard the current target before the resize and waits for one new-surface frame after the no-redraw, no-copy-bits window position update. The live swap chain opts out of stale-buffer stretching during that boundary.
- Let the live presentation backend pace active animation frames; the render thread does not keep a fixed animation-frame timer.
