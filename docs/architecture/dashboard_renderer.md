...
- Treat HWND size changes as synchronized presentation boundaries: the shell waits for the render thread to discard the current target before the resize and waits for one new-surface frame after the no-redraw window position update.
