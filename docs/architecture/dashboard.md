...
- `DashboardApp` owns active auto-hide drawer state, including exact placement-target eligibility, topmost state, hidden-edge pointer polling, close-delay timing, native slide show or hide animation, and titlebar suppression while auto-hide is active. The drawer animation runs against a short-lived topmost layered snapshot window because direct `AnimateWindow` calls on the live GPU-backed dashboard HWND do not reliably produce visible slide content.
...
- `DashboardController` owns active config state, runtime instance lifetime, diagnostics session lifetime, save actions, layout and scale switching, runtime target selection, layout-edit session state, and one-shot config updates when move or resize placement completes.
