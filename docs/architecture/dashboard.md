...
- `DashboardApp` owns active autohide drawer state, including exact placement-target eligibility, topmost state, hidden-edge pointer polling, close-delay timing, native slide show or hide animation, and titlebar suppression while autohide is active. The drawer animation runs against a short-lived topmost layered snapshot window because direct `AnimateWindow` calls on the live GPU-backed dashboard HWND do not reliably produce visible slide content.
