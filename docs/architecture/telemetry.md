...
- Native vendor providers keep provider-specific setup and method translation inside the provider module, including contracts such as ASUS ATKACPI `DSTS` device calls, Lenovo Hardware Scan LdeApi temperature calls, Lenovo GameZone WMI fan calls, and vendor SDK bridges, then publish normal board-provider samples to the collector. Slow direct provider refresh paths own their own cadence and cached sample state so telemetry startup does not wait on vendor diagnostics scans.
