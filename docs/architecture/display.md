...
- Compute fullscreen and edge placement targets in one shared helper used by configure-display menu options, saved display config updates, committed active-row matching, and auto-hide runtime eligibility checks.
...
- Provide the pure aspect-preserving interactive resize scale and corner-anchored target helpers used by dashboard resize mode before committing explicit display scale and placement.
...
- Support wallpaper and configure-display helpers used by dashboard shell commands. Fullscreen choices render and apply the CaseDash blank wallpaper and clear `[display] autohide`; edge placements save config without creating new wallpaper, set `[display] autohide` to the selected edge, and clear previous committed CaseDash wallpaper ownership.

## Boundaries

- Display owns monitor and placement primitives; dashboard shell owns user command presentation.
- Runtime behavior for display selection, move mode, and corner resize mode is specified in [docs/specifications.md](../specifications.md).
