...
- `.clang-format` is the single maintained source of truth for C++ formatting policy. The golden formatter fixture under `tools\tests\format\src` stays synchronized with every configured formatting option. `format.cmd` owns narrow exclusions for mixed-mode C++/CLI bridge `.cpp` files that clang-format versions do not format consistently.
