...
- Keep the focused performance benchmark host in the native build as `SystemTelemetryBenchmarks` so both the `edit-layout` drag path and the `update-telemetry` steady-state snapshot-refresh path are measured against the real controller, config-apply, live telemetry collector update, and renderer paint flow instead of against a synthetic relayout-only loop.
