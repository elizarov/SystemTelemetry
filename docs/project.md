...
- `docs/hardware-dependency-interface.md` owns the hardware dependency interface for testable hardware-provider dependencies.
...
- Topmost state is not the same as topmost z-order freshness; after the Windows taskbar is clicked, autohide drawer show paths must reissue `SetWindowPos(HWND_TOPMOST, ...)` so the dashboard returns above other topmost windows.
