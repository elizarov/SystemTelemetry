...
- Use the top-level `profile_benchmark.cmd` script as the maintained entry point for benchmark CPU profiling; it writes the benchmark ETL, flat CPU summary, and benchmark-only butterfly call tree HTML export into `build\`, accepts optional `iterations` and `render_scale` positional arguments, accepts `/daemon-start`, `/daemon-stop`, and `/daemon-status` to manage one elevated benchmark daemon under `build\profile_benchmark_daemon\`, hands off ordinary benchmark runs to that daemon when it is ready, and otherwise accepts `/elevate` to relaunch a one-shot elevated run when WPR profiling needs `SeSystemProfilePrivilege`.
