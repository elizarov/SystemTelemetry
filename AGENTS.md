...
- `profile_benchmark.cmd` supports `/daemon-start`; start that elevated helper once when unattended benchmark runs need to be triggered repeatedly from the sandbox, then let ordinary benchmark invocations queue through the daemon instead of prompting for UAC each time.
