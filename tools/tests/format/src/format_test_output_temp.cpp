...
void WriteTraceStringFragments(TraceLog& trace) {
    trace.WriteFmt(TracePrefix::Diagnostics, RES_STR("layout_guide_sheet stats selected_cards=%zu callouts=%zu"), stats.selectedCards, stats.callouts);
}
...
void WriteLongTraceStringFragments(TraceLog& trace, const char* adapterName) {
    trace.WriteFmt(TracePrefix::GpuVendor, RES_STR("adapter_candidate index=%u vendor_id=0x%04X device_id=0x%04X subsystem_id=0x%08X " "luid=0x%08x:0x%08x pci=%04X:%02X:%02X.%u vendor=%s match_rank=%d dedicated_gb=%.2f " "name=\"%s\""), adapterIndex, adapterName);
}

std::string FormatLayoutEditTraceText(const LayoutEditTraceState& state, const char* captureText) {
    std::string trace = FormatText("layout=\"%s\" editing=%s moving=%s modal_depth=%d tooltip_visible=%s tooltip_suppressed=%s " "tooltip_rect_valid=%s mouse_tracking=%s drag_active=%s capture=\"%s\"", state.config.display.layout.c_str(), Trace::BoolText(state.isEditingLayout), Trace::BoolText(state.isMoving), layoutEditModalUiDepth_, Trace::BoolText(dashboardTooltipOwner_ == DashboardTooltipOwner::LayoutEdit && dashboardTooltip_.Visible()), Trace::BoolText(layoutEditTooltipRefreshSuppressed_), Trace::BoolText(dashboardTooltipOwner_ == DashboardTooltipOwner::LayoutEdit && dashboardTooltip_.TargetRectValid()), Trace::BoolText(layoutEditMouseTracking_), Trace::BoolText(layoutEditController_.HasActiveDrag()), captureText);
    return trace;
}
...
OklabColor MixOklab(OklabColor from, OklabColor to, double amount) {
    return OklabColor{from.l + (to.l - from.l) * amount, from.a + (to.a - from.a) * amount, from.b + (to.b - from.b) * amount};
}
...
OklchColor NormalizeOklch(const double* lightnessOverrideWithLongName, const double* chromaOverrideWithLongName, const double* hueOverrideWithLongName) {
    return OklchColor{std::clamp(*lightnessOverrideWithLongName, 0.0, 1.0), std::max(0.0, *chromaOverrideWithLongName), std::clamp(*hueOverrideWithLongName, 0.0, 360.0)};
}
...
void SaveBoardSectionDifferences(const BoardConfig& board, const BoardConfig* compareBoard, const std::string& sectionName, UpdateKeyFn& updateKey) {
    DynamicSectionSaveContext<UpdateKeyFn> context{&board, compareBoard, &updateKey};
    updateKey(board, compareBoard, sectionName);
}

template <typename FirstTemplateParameter, typename SecondTemplateParameter, typename ThirdTemplateParameter, typename FourthTemplateParameter>
struct LongTemplateParameterHost {};

template <typename Result, typename... Args>
class FunctionRef<Result(Args...)> {
public:
    template <typename Callable> requires(!std::is_same_v<std::remove_cvref_t<Callable>, FunctionRef> && std::is_invocable_r_v<Result, Callable&&, Args...>)
    FunctionRef(Callable&& callable) : context_(const_cast<void*>(static_cast<const void*>(std::addressof(callable)))), invoke_([](void* context, Args... args) -> Result {
...
using RuntimeConfigForEachDynamicItem = void (*)(const AppConfig& config, void* context, RuntimeConfigDynamicItemVisitor visitor);
...
void RegisterStaticTextAnchor(const RenderRect&, const std::string&, TextStyleId, const TextLayoutOptions&, const LayoutEditAnchorBinding&, std::optional<LayoutEditParameter>, LayoutEditTargetOutline) override {}
...
HBITMAP CreateBitmap(BITMAPINFOHEADER header) {
    void* bits = nullptr;
    HBITMAP colorBitmap = CreateDIBSection(nullptr, reinterpret_cast<BITMAPINFO*>(&header), DIB_RGB_COLORS, &bits, nullptr, 0);
    return colorBitmap;
}
...
const auto noLookup = [](std::string_view) -> std::optional<ColorConfig> {
    return std::nullopt;
};
...
const auto appendTrialLeaders = [&](std::vector<TrialLeader>& leaders, const std::vector<size_t>& plannedIndexes, LayoutGuideSheetExitSide side, const LayoutGuideSheetCardPlacement& placement, const BlockLayout& block) {
    Use(leaders, plannedIndexes, side, placement, block);
};

const auto appendEmbeddedRect = [&](const RenderRect& rect, const std::vector<LayoutEditOverlayOwner>* owners, LayoutEditOverlayAffordanceLayer artifactLayer) {
    if (rect.IsEmpty()) {
        return;
    }
...
const auto updateKey = [&lines, &ensureSection, &ensureSectionAfter, &findSectionEnd, shape](const std::string& sectionName, const std::string& key, const std::string& value) {
    Use(sectionName, key, value);
};

const auto ensureSectionAfter = [&lines, &findSectionIndex, shape](const std::string& sectionName, const std::string& afterSectionName) -> size_t {
    const size_t existingIndex = findSectionIndex(sectionName);
    if (existingIndex < lines.size()) {
        return existingIndex;
    }
...
const auto guideSheetLookup = [&config, activeTheme, &colorsSection](std::string_view name) -> std::optional<ColorConfig> {
...
void DiagnosticsSession::ResolveOutputPathMember(const OutputPath& outputPath, const FilePath& workingDirectory) {
    this->*outputPath.resolvedPath = ResolveDiagnosticsOutputPath(workingDirectory, options_.*outputPath.configuredPath, outputPath.defaultFileName);
}
...
class BenchmarkLikeHost {
    bool ApplyMetricListOrder(const LayoutEditWidgetIdentity& widget, const std_fixture::vector<std_fixture::string>& metricRefs) override {
        return true;
    }
};
...
void PreferOuterCallBreakForSingleArgument() {
    UseAVeryLongCallNameThatPushesPastLimit(DecorateValueWithLongName(firstValueWithLongName && SingleArgumentCallWithLongName(secondValueWithLongName)));
}
...
void BuildTitlebarTooltipControls() {
    ...
    controls[] = {{DashboardTitlebarTooltipControl::Close, closeRect}, {DashboardTitlebarTooltipControl::Display, displayRect}, {DashboardTitlebarTooltipControl::EditLayout, editLayoutRect}, {DashboardTitlebarTooltipControl::Layout, layoutComboRect}, {DashboardTitlebarTooltipControl::Theme, themeComboRect}, {DashboardTitlebarTooltipControl::AppMenu, appMenuRect}};
}
...
std::string SelectCurrentSensorName(const BoardConfig& board, const std::string& logicalName) {
    ...
    const std::string currentValue = currentIt != board.temperatureSensorNames.end() && !currentIt->second.empty() ? currentIt->second : logicalName;
    return currentValue;
}
...
bool IsNamedColorField(const RuntimeConfigFieldDescriptor& field, std::string_view name) {
    if (field.kind == RuntimeConfigFieldValueKind::HexColor && std::string_view(field.key, field.keyLength) == name && field.keyLength > 0) {
        return true;
    }
    return false;
}
...
bool HasMissingReflectionMembers(const ReflectionContext* context) {
    if (context->initializeMethod == nullptr || context->getCurrentMethod == nullptr || context->titleProperty == nullptr || context->valueProperty == nullptr) {
        return true;
    }
    return false;
}
...
void CollectLayoutEditHighlights(const DashboardOverlayState& overlayState, const LayoutEditAnchorRegion& region) {
    ...
    if (MatchesLayoutEditSelectionHighlight(*overlayState.selectedTreeHighlight, region.key) || (special != nullptr && *special == LayoutEditSelectionHighlightSpecial::AllTexts && LayoutEditAnchorParameter(region.key).has_value() && IsFontEditParameter(*LayoutEditAnchorParameter(region.key)))) {
...
void FormatterSelfBreakCases() {
    if (tokens[statementStart].text == "for" || tokens[statementStart].text == "while" || tokens[statementStart].text == "switch") {
        return;
    }
    if (next < tokens.size() && tokens[next].kind == TokenKind::Word && (tokens[next].text == "else" || tokens[next].text == "catch" || tokens[next].text == "finally" || (tokens[next].text == "while" && closedBlock.kind == BlockKind::DoStatement))) {
        return;
    }
    if (pendingTokens_.empty() && pendingPrefix_.empty() && IsTrailingCommentAfterEmittedClose(tokens, index) && !outputLines_.empty()) {
        return;
    }
}
...
bool ConfigureDisplayGuard(DisplayState& state, DisplayOption option, DashboardShellHost& shell, UpdatedConfig updatedConfig) {
    if (!::ConfigureDisplay(updatedConfig, state.telemetryUpdate.dump, option.fittedScale, shell.TraceLog(), shell.WindowHandle())) {
        return true;
    }
    return false;
}
...
void PlaceEmptyLambdaCallout() {
    const LayoutGuideSheetPlacementResult result = PlaceLayoutGuideSheetCallouts(cardPlacements, callouts, LayoutGuideSheetPlacementStyle{10, 12, 4, 20, 0, 1}, [](LayoutGuideSheetPlacementCallout&, int) {}, nullptr);
}

void SnapGaugeWidth() {
    const bool snapped = layout_snap_solver::FindNearestSnapWeight(kCurrentGaugeWeight, kCombinedWeight, kThreshold, {layout_snap_solver::SnapCandidate{targetExtent, targetExtent - startExtent, 0}}, [](int firstWeight, int& extent) -> bool {
...
int CountPlusAnchors(RenderHost& renderer) {
    return static_cast<int>(std::count_if(renderer.editArtifacts.staticAnchors.begin(), renderer.editArtifacts.staticAnchors.end(), [](const LayoutEditAnchorRegion& region) {
...
void LongForCondition() {
    for (int rowIndex = 0; rowIndex < layoutState_.visibleRows && rowIndex < static_cast<int>(layoutState_.rowBarRects.size()) && rowIndex < static_cast<int>(layoutState_.rowBarAnchorRects.size()); ++rowIndex) {
        Use(rowIndex);
    }
}
