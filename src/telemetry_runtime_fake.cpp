...
public:
    explicit FakeTelemetryRuntime(std::filesystem::path fakePath) : fakePath_(std::move(fakePath)) {}
...
std::unique_ptr<TelemetryRuntime> CreateTelemetryRuntime(
    ...
    if (options.fake) {
        return std::make_unique<FakeTelemetryRuntime>(executableDirectory / L"telemetry_fake.txt");
    }
