...
std::string DescribeWidgetParameter(LayoutEditParameter parameter) {
    ...
    return FormatText("%s.%s", metadata.sectionName, metadata.parameterName);
}
...
std::optional<int> ContainerChildSlotIndexForCoordinate(
    ...
    for (int index = 0; index < static_cast<int>(childRects.size()); ++index) {
        ...
        const int start = ContainerChildAxisStart(rect, horizontal);
        const int end = ContainerChildAxisEnd(rect, horizontal);
        if (coordinate < start) {
            return lastSlot.has_value() ? std::nullopt : std::optional<int>(index);
        }
        if (coordinate < end) {
            return index;
        }
...
void LayoutEditController::CancelInteraction() {
    const bool hadInteraction = hoveredLayoutGuide_.has_value() || hoveredLayoutCard_.has_value() ||
        hoveredEditableCard_.has_value() || hoveredEditableWidget_.has_value() ||
        hoveredGapEditAnchorRegion_.has_value() || hoveredGapEditAnchor_.has_value() ||
        hoveredWidgetEditGuide_.has_value() || hoveredEditableAnchor_.has_value() || activeLayoutDrag_.has_value() ||
        activeWidgetEditDrag_.has_value() || activeGapEditDrag_.has_value() || activeAnchorEditDrag_.has_value() ||
        ...
        if (!layout_snap_solver::FindNearestSnapWeight(
                ...
                [&](int firstWeight, int& extent) -> bool {
                    ...
                    const auto cached = std::find_if(
                        drag.extentCache.begin(), drag.extentCache.end(), [&](const ExtentCacheEntry& entry) {
...
bool LayoutEditController::UpdateContainerChildReorderDrag(RenderPoint clientPoint) {
    ...
    drag.mouseCoordinate = drag.horizontal ? clientPoint.x : clientPoint.y;
    if (drag.stableChildRects.size() != static_cast<size_t>((std::max)(0, drag.childCount))) {
        SyncRendererInteractionState();
        host_.InvalidateLayoutEdit();
        return true;
    }

    const std::optional<int> targetSlot =
        ContainerChildSlotIndexForCoordinate(drag.stableChildRects, drag.horizontal, drag.mouseCoordinate);
    ...
    const int targetIndex = std::clamp(*targetSlot, 0, (std::max)(0, drag.childCount - 1));
    if (targetIndex == drag.currentIndex) {
        SyncRendererInteractionState();
        host_.InvalidateLayoutEdit();
        return true;
