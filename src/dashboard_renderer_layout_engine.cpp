...
void DashboardRendererLayoutEngine::BuildWidgetEditGuides(DashboardRenderer& renderer) {
    const auto addMetricListWidgetEditGuides = [&](const DashboardRenderer::ResolvedWidgetLayout& widget) {
        ...
        const int totalRows =
            widget.binding.param.empty()
                ? 0
                : 1 + static_cast<int>(std::count(widget.binding.param.begin(), widget.binding.param.end(), ','));
        const int visibleRows =
            rowHeight > 0
                ? std::clamp(((std::max)(0, static_cast<int>(widget.rect.bottom - widget.rect.top)) + rowHeight - 1) /
                                 rowHeight,
                      0,
                      totalRows)
                : 0;

        const auto addGuide = [&](DashboardRenderer::LayoutGuideAxis axis,
                                  int guideId,
            ...
            guide.drawEnd = axis == DashboardRenderer::LayoutGuideAxis::Vertical ? POINT{lineRect.left, lineRect.bottom}
        ...
        addGuide(DashboardRenderer::LayoutGuideAxis::Vertical,
            0,
            ...
            renderer.config_.layout.metricList.labelWidth,
            1);

        for (int rowIndex = 0; rowIndex < visibleRows; ++rowIndex) {
            const int y = widget.rect.top + ((rowIndex + 1) * rowHeight);
            addGuide(DashboardRenderer::LayoutGuideAxis::Horizontal,
                1 + rowIndex,
                ...
                renderer.config_.layout.metricList.verticalGap,
                1);
        }
    };
    ...
    const auto addDriveUsageWidgetEditGuides = [&](const DashboardRenderer::ResolvedWidgetLayout& widget) {
        ...
        const int activityWidth =
            (std::max)(1, renderer.ScaleLogical(renderer.config_.layout.driveUsageList.activityWidth));
        ...
        const int availableRowPixels =
            (std::max)(0, static_cast<int>(widget.rect.bottom - widget.rect.top) - headerHeight);
        const int visibleRows =
        ...
        RECT barRect{(std::min)(widget.rect.right, static_cast<LONG>(writeRect.right + barGap)),
            ...
            (std::max)((std::min)(widget.rect.right, static_cast<LONG>(writeRect.right + barGap)),
                static_cast<LONG>(
                    pctRect.left - renderer.ScaleLogical(renderer.config_.layout.driveUsageList.percentGap))),
            widget.rect.bottom};
        ...
        const auto addVerticalGuide =
            [&](int guideId, int x, DashboardRenderer::WidgetEditParameter parameter, int value, int dragDirection) {
                const int clampedX =
                    std::clamp(x, static_cast<int>(widget.rect.left), static_cast<int>(widget.rect.right));
                DashboardRenderer::WidgetEditGuide guide;
        ...
        if (visibleRows > 0 && renderer.config_.layout.driveUsageList.activitySegments > 1) {
            ...
            const int rowContentHeight = (std::max)(renderer.fontHeights_.label,
                (std::max)(renderer.fontHeights_.smallText,
                    renderer.ScaleLogical(renderer.config_.layout.driveUsageList.barHeight)));
            const int activitySegmentGap = ClampStackedSegmentGap(rowContentHeight,
            ...
            const int contentTop =
                widget.rect.top + headerHeight + (std::max)(0, (rowPixelHeight - rowContentHeight) / 2);
            const RECT activityBandRect{readRect.left, contentTop, writeRect.right, contentTop + rowContentHeight};
            const int lowestSegmentTop = ComputeLowestStackedSegmentTop(activityBandRect.top,
                rowContentHeight,
                activityWidth,
                renderer.config_.layout.driveUsageList.activitySegments,
                activitySegmentGap);
            addHorizontalGuide(7,
                ...
                renderer.config_.layout.driveUsageList.activitySegmentGap,
                1,
        ...
        for (int rowIndex = 0; rowIndex < visibleRows; ++rowIndex) {
            ...
            addHorizontalGuide(8 + rowIndex,
                ...
                renderer.config_.layout.driveUsageList.rowGap,
                1);
        }
    };
    ...
    const auto addThroughputWidgetEditGuide = [&](const DashboardRenderer::ResolvedWidgetLayout& widget) {
        ...
        RECT graphRect{widget.rect.left,
            (std::min)(widget.rect.bottom,
                valueRect.bottom + (std::max)(0, renderer.ScaleLogical(renderer.config_.layout.throughput.headerGap))),
            widget.rect.right,
            widget.rect.bottom};
        ...
        addGuide(DashboardRenderer::LayoutGuideAxis::Vertical,
            ...
            renderer.config_.layout.throughput.axisPadding,
            1);
        ...
        addGuide(DashboardRenderer::LayoutGuideAxis::Horizontal,
            ...
            RECT{widget.rect.left, y, widget.rect.right, y + 1},
            RECT{widget.rect.left, y - hitInset, widget.rect.right, y + hitInset + 1},
            renderer.config_.layout.throughput.headerGap,
            1);
    };

    const auto addGaugeWidgetEditGuide = [&](const DashboardRenderer::ResolvedWidgetLayout& widget) {
        const int radius = (std::min)(
            (std::max)(1, renderer.resolvedLayout_.globalGaugeRadius), renderer.GaugeRadiusForRect(widget.rect));
        if (radius <= 0) {
            return;
        }
        ...
        const GaugeSegmentLayout gaugeLayout = ComputeGaugeSegmentLayout(renderer.config_.layout.gauge.sweepDegrees,
