...
DashboardLayoutResolver::DashboardLayoutResolver(DashboardRenderer& renderer) :
    renderer_(renderer) {}
    ...
    for (const ContainerChildReorderTarget& target : containerChildReorderTargets_) {
        ...
        for (size_t i = 0; i < target.childRects.size(); ++i) {
            ...
            RegisterStaticEditAnchor(LayoutEditAnchorRegistration{
                .key = LayoutEditAnchorKey{widgetIdentity,
...
bool DashboardLayoutResolver::ResolveLayout(DashboardRenderer& renderer, bool includeWidgetState) {
    ...
    const auto resolveCard =
        [&](const LayoutNodeConfig& node,
            ...
            if (writeTrace) {
                renderer.WriteTraceFmt(
                    RES_STR("layout_card id=\"%s\" rect=(%d,%d,%d,%d) title=rect=(%d,%d,%d,%d) "
    ...
    const auto resolveDashboardNode =
        ...
        for (size_t i = 0; i < node.children.size(); ++i) {
            ...
            if (writeTrace) {
                renderer.WriteTraceFmt(
                    RES_STR("layout_dashboard_child parent=\"%s\" child=\"%s\" weight=%d gap=%d "
                            "size=%d rect=(%d,%d,%d,%d)"),
                    node.name.c_str(),
                    child.name.c_str(),
                    childWeight,
