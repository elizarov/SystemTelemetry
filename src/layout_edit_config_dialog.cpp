...
struct LayoutEditConfigDialogState {
    LayoutEditConfigDialog* dialog = nullptr;
...
void ConfigureDialogFonts(LayoutEditConfigDialogState* state, HWND hwnd) {
    if (state == nullptr) {
        return;
    }
...
void SetLayoutEditStatus(
    LayoutEditConfigDialogState* state, HWND hwnd, LayoutEditStatusKind kind, const std::wstring& text) {
    if (state == nullptr) {
        return;
    }
...
void SetColorSamplePreview(LayoutEditConfigDialogState* state, HWND hwnd, unsigned int color) {
    if (state == nullptr) {
        return;
    }
...
void SetFontSamplePreview(LayoutEditConfigDialogState* state,
    HWND hwnd,
...
LayoutEditEditorKind CurrentLayoutEditEditorKind(const LayoutEditConfigDialogState* state) {
...
bool CurrentLayoutEditShowsMetricBinding(const LayoutEditConfigDialogState* state) {
    if (state == nullptr || state->selectedLeaf == nullptr) {
        return false;
    }
...
void LayoutLayoutEditRightPane(LayoutEditConfigDialogState* state, HWND hwnd) {
    if (state == nullptr) {
        return;
    }
...
LayoutEditConfigDialogState* DialogStateFromWindow(HWND hwnd) {
    return reinterpret_cast<LayoutEditConfigDialogState*>(GetWindowLongPtrW(hwnd, DWLP_USER));
}

void UpdateLayoutEditActionState(LayoutEditConfigDialogState* state, HWND hwnd) {
...
void SelectLayoutEditTreeItem(LayoutEditConfigDialogState* state, HWND hwnd, HTREEITEM item);

void PopulateLayoutEditSelection(LayoutEditConfigDialogState* state, HWND hwnd) {
    if (state == nullptr) {
        return;
    }
...
LayoutEditValidationResult ValidateCurrentSelectionInput(LayoutEditConfigDialogState* state, HWND hwnd) {
    if (state == nullptr || state->selectedLeaf == nullptr) {
        return {true, L""};
    }
...
void RefreshLayoutEditValidationState(LayoutEditConfigDialogState* state, HWND hwnd) {
    if (state == nullptr) {
        return;
    }
...
bool PreviewSelectedValue(LayoutEditConfigDialogState* state, HWND hwnd) {
...
bool PreviewSelectedFont(LayoutEditConfigDialogState* state, HWND hwnd, UINT notificationCode = 0) {
    if (state == nullptr || state->selectedLeaf == nullptr || state->updatingControls) {
        return false;
    }
    const auto* parameter = std::get_if<LayoutEditParameter>(&state->selectedLeaf->focusKey);
...
bool PreviewSelectedColor(LayoutEditConfigDialogState* state, HWND hwnd) {
    if (state == nullptr || state->selectedLeaf == nullptr || state->updatingControls) {
        return false;
    }
    const auto* parameter = std::get_if<LayoutEditParameter>(&state->selectedLeaf->focusKey);
...
bool SetSelectedDialogColor(LayoutEditConfigDialogState* state, HWND hwnd, unsigned int color) {
    if (state == nullptr || state->selectedLeaf == nullptr) {
        return false;
    }
    const auto* parameter = std::get_if<LayoutEditParameter>(&state->selectedLeaf->focusKey);
...
bool PreviewSelectedWeights(LayoutEditConfigDialogState* state, HWND hwnd) {
    if (state == nullptr || state->selectedLeaf == nullptr || state->updatingControls) {
        return false;
    }
...
bool PreviewSelectedMetric(LayoutEditConfigDialogState* state, HWND hwnd) {
    if (state == nullptr || state->selectedLeaf == nullptr || state->updatingControls) {
        return false;
    }
...
void InsertLayoutEditTreeNodes(
    LayoutEditConfigDialogState* state, HWND tree, const std::vector<LayoutEditTreeNode>& nodes, HTREEITEM parent) {
...
HTREEITEM FindTreeItemByFocusKey(LayoutEditConfigDialogState* state, const LayoutEditFocusKey& focusKey) {
...
HTREEITEM FindTreeItemByLocationText(LayoutEditConfigDialogState* state, std::string_view locationText) {
    for (const auto& binding : state->treeItems) {
...
HTREEITEM FindFirstLeafTreeItem(const LayoutEditConfigDialogState& state) {
    for (const auto& binding : state.treeItems) {
        if (binding.node != nullptr && binding.node->leaf.has_value()) {
            return binding.item;
        }
...
HTREEITEM FindFirstTreeItem(const LayoutEditConfigDialogState& state) {
...
void RebuildLayoutEditTree(LayoutEditConfigDialogState* state,
    HWND hwnd,
    const std::optional<LayoutEditFocusKey>& preferredFocus = std::nullopt) {
    if (state == nullptr) {
        return;
    }
...
void RefreshLayoutEditDialogControls(LayoutEditConfigDialogState* state,
    HWND hwnd,
...
bool RevertSelectedLayoutEditField(LayoutEditConfigDialogState* state, HWND hwnd) {
    if (state == nullptr || state->selectedLeaf == nullptr) {
        return false;
    }
...
LayoutEditConfigDialog::LayoutEditConfigDialog(LayoutEditConfigDialogHost& host) : host_(host) {}

LayoutEditConfigDialog::~LayoutEditConfigDialog() {
    Close();
}

bool LayoutEditConfigDialog::HandleDialogMessage(MSG* msg) const {
    return msg != nullptr && hwnd_ != nullptr && IsWindow(hwnd_) && IsDialogMessageW(hwnd_, msg) != FALSE;
}

bool LayoutEditConfigDialog::IsForegroundWindow() const {
...
void LayoutEditConfigDialog::BringToFront() {
    if (hwnd_ == nullptr || !IsWindow(hwnd_)) {
        return;
    }
...
void LayoutEditConfigDialog::PositionWindow(HWND hwnd) const {
...
void LayoutEditConfigDialog::UpdateSelectionHighlight(const std::optional<LayoutEditSelectionHighlight>& highlight) {
    selectionHighlight_ = highlight;
    ApplySelectionHighlightVisibility();
}

void LayoutEditConfigDialog::ApplySelectionHighlightVisibility() {
    host_.UpdateLayoutEditSelectionHighlight(selectionHighlightVisible_ ? selectionHighlight_ : std::nullopt);
}

void LayoutEditConfigDialog::SetSelectionHighlightVisible(bool visible) {
...
void LayoutEditConfigDialog::HandleDestroyed(HWND hwnd) {
    if (hwnd_ == hwnd) {
        hwnd_ = nullptr;
    }
...
void LayoutEditConfigDialog::Close() {
    if (hwnd_ == nullptr) {
...
bool LayoutEditConfigDialog::Ensure(const std::optional<LayoutEditFocusKey>& focusKey, bool bringToFront) {
    if (hwnd_ != nullptr && IsWindow(hwnd_)) {
        Refresh(focusKey);
        if (bringToFront) {
            BringToFront();
    ...
    state_ = std::make_unique<LayoutEditConfigDialogState>();
    state_->dialog = this;
    ...
    HWND dialog = CreateDialogParamW(host_.LayoutEditDialogInstance(),
        ...
        nullptr,
        LayoutEditConfigDialog::DialogProc,
...
void LayoutEditConfigDialog::Refresh(const std::optional<LayoutEditFocusKey>& preferredFocus) {
...
void LayoutEditConfigDialog::RefreshSelection() {
    if (hwnd_ == nullptr || !IsWindow(hwnd_) || state_ == nullptr) {
        return;
    }
...
bool LayoutEditConfigDialog::SyncSelection(
...
bool LayoutEditConfigDialog::ShouldDashboardIgnoreMouse(POINT screenPoint) const {
...
INT_PTR CALLBACK LayoutEditConfigDialog::DialogProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    auto* state = DialogStateFromWindow(hwnd);
    switch (message) {
        case WM_INITDIALOG: {
            state = reinterpret_cast<LayoutEditConfigDialogState*>(lParam);
