...
array<unsigned char>^ MsiCenterCurrentDataCommand() {
    return gcnew array<unsigned char>{0x05, 0x03, 0x01, 0x08, 0x01, 0x00, 0x00, 0x01};
}
...
String^ ReadMsiFanName(Object^ fanName, FieldInfo^ fanNameBytesField, FieldInfo^ fanNameLengthField, int fallbackId) {
...
bool InitializeMsiCenterRuntime(
    ...
    try {
        ...
        context->sendDataMethod = context->clientType->GetMethod(
            "SendData",
        ...
        if (
            context->sendDataMethod == nullptr ||
            context->jsonDeserializerMethod == nullptr ||
            context->fanCountsField == nullptr ||
            context->curFanIdField == nullptr ||
            context->curFanSpeedField == nullptr ||
            context->fanNameField == nullptr ||
            context->fanNameBytesField == nullptr ||
            context->fanNameLengthField == nullptr ||
            context->temperatureCountsField == nullptr ||
            context->curTempIdField == nullptr ||
...
template <typename T>
T GetFieldValueOr(FieldInfo^ field, Object^ owner, T fallback) {
