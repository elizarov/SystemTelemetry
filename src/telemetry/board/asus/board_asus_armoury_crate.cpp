...
void TraceAtkDriverResult(Trace& trace,
    ...
    DWORD bytesReturned,
    std::uint32_t status) {
    trace.WriteLazy(TracePrefix::AsusArmouryCrate, [&] {
        char idText[16];
        char statusText[16];
        sprintf_s(idText, "0x%08x", deviceId);
        sprintf_s(statusText, "0x%08x", status);
        std::string text = std::string("atk_driver_") + kind + " id=" + idText;
        if (name != nullptr && name[0] != '\0') {
            text += " name=\"";
            text += name;
            text += "\"";
        }
        text += " ok=" + std::to_string(ok ? 1 : 0) + " error=" + std::to_string(error) +
                " bytes=" + std::to_string(bytesReturned) + " status=" + statusText;
        return text;
    });
}
...
AsusArmouryCrateSnapshot CaptureAsusAtkDriverSensors(Trace& trace) {
    ...
    if (!device.Valid()) {
        ...
        snapshot.diagnostics = "ASUS ATKACPI device open failed: " + std::to_string(error);
        trace.Write(TracePrefix::AsusArmouryCrate, "atk_driver_open_failed error=" + std::to_string(error));
        return snapshot;
    }
    ...
    snapshot.diagnostics =
        "ASUS Armoury Crate ATKACPI query completed. fan_count=" + std::to_string(snapshot.fans.size()) +
        " temp_count=" + std::to_string(snapshot.temperatures.size());
    trace.WriteLazy(TracePrefix::AsusArmouryCrate, [&] {
        return "atk_driver_snapshot_done fan_count=" + std::to_string(snapshot.fans.size()) +
               " temp_count=" + std::to_string(snapshot.temperatures.size());
    });
    return snapshot;
}
...
public:
    ...
    bool Initialize(const BoardTelemetrySettings& settings) override {
        ...
        trace_.Write(TracePrefix::AsusArmouryCrate,
